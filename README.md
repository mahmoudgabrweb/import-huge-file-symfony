...
## For running code
- Add two files in public folder
  * First file is the logs file and should be in the name of ```logs.txt```
  * Second file is ```updates.txt``` which is keeping the track of last scanned file
- For running the command ```symfony app:import-logs```